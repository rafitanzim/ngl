...
print("Made By :Tanzim Khan Rafi")
print("\n[{}] >> Starting\n".format(ido))
if hossz > 0:
  fiokok = []
  kerdesek = []
  ...
  while x != ismetles:
    if i < 10:
      ...
      if elkuld.status_code == 200:
        ...
        if nemsikerult < 4:
          print("[{} ({}/3)] Failed! I'll try again after 10 seconds.\n".format(elkuld.status_code,nemsikerult))
          time.sleep(10)
        else:
          datum = datetime.now()
  ...
  while True:
    if (i < 10):
      ...
      if elkuld.status_code == 200:
        ...
        nemsikerult = nemsikerult + 1
        if nemsikerult < 4:
          print("[{} ({}/3)] Failed! I'll try again after 5 seconds.\n".format(elkuld.status_code,nemsikerult))
          time.sleep(5)
        else:
          datum = datetime.now()
          datumido = datum.strftime("%Y-%m-%d")
